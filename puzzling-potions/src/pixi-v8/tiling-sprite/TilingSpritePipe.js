...
var QuadGeometry = require('./QuadGeometry.js');
...
class TilingSpritePipe {
  ...
  initGpuTilingSprite(renderable) {
    ...
    view.texture.style.addressMode = "repeat";
    view.texture.style.update();
    const meshView = new MeshView.MeshView({
      geometry: sharedQuad,
      shader: new TilingSpriteShader.TilingSpriteShader({
        texture: view.texture
      })
    });
    const meshRenderable = new ProxyRenderable.ProxyRenderable({
      original: renderable,
      view: meshView
    });
    const textureMatrix = new Matrix.Matrix();
  ...
  initBatchedTilingSprite(renderable) {
    const meshView = new MeshView.MeshView({
      geometry: new QuadGeometry.QuadGeometry(),
      texture: renderable.view.texture
    });
    const batchableMeshRenderable = new ProxyRenderable.ProxyRenderable({
      original: renderable,
      view: meshView
    });
