...
var WebGPURenderer = require('../../WebGPURenderer.js');
...
function textureToCanvasWebGL(texture, renderer) {
  ...
  const canvas2 = settings.settings.ADAPTER.createCanvas();
  canvas2.width = width;
...
async function logDebugTexture(texture, renderer, size = 200) {
  let canvas;
  if (renderer instanceof WebGPURenderer.WebGPURenderer) {
...
exports.textureToCanvasWebGL = textureToCanvasWebGL;
