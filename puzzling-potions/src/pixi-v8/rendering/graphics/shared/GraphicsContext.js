...
const _GraphicsContext = class extends EventEmitter__default["default"] {
  constructor() {
    super(...arguments);
    this.uid = UID++;
    this.usage = 0;
    ...
    this.batchMode = "auto";
    this.instructions = [];
    this.activePath = new GraphicsPath.GraphicsPath();
  ...
  set fillStyle(value) {
    ...
    if (typeof value === "number" || typeof value === "string") {
      this._fillStyle.color = convertColorToNumber.convertColorToNumber(value);
      this._fillStyle.texture = Texture.Texture.WHITE;
    } else if (value instanceof FillPattern.FillPattern) {
      const pattern = value;
      this._fillStyle.color = 16777215;
      this._fillStyle.texture = pattern.texture;
      this._fillStyle.matrix = pattern.transform;
    } else if (value instanceof FillGradient.FillGradient) {
  ...
  set strokeStyle(value) {
    ...
    if (typeof value === "number" || typeof value === "string") {
      this._strokeStyle.color = convertColorToNumber.convertColorToNumber(value);
      this._strokeStyle.texture = Texture.Texture.WHITE;
    } else if (value instanceof FillGradient.FillGradient) {
      const gradient = value;
      gradient.buildLinearGradient();
  ...
  fill(style) {
    ...
    if (style) {
      fillStyle = convertFillInputToFillStyle.convertFillInputToFillStyle(style, _GraphicsContext.defaultFillStyle);
    }
    this.instructions.push({
      action: "fill",
  ...
  stroke(style) {
    ...
    if (style) {
      strokeStyle = convertFillInputToFillStyle.convertFillInputToFillStyle(style, _GraphicsContext.defaultStrokeStyle);
    }
    this.instructions.push({
      action: "stroke",
      // TODO copy fill style!
