...
var BitmapFontManager = require('./bitmap/BitmapFontManager.js');
...
const _TextView = class {
  constructor(options) {
    ...
    this._style = options.style instanceof TextStyle.TextStyle ? options.style : new TextStyle.TextStyle(options.style);
    const renderMode = options.renderMode ?? this.detectRenderType(this._style);
    this.type = map[renderMode];
    this.anchor = new ObservablePoint.ObservablePoint(this, 0, 0);
  ...
  updateBounds() {
    ...
    if (this.type === "bitmapText") {
      ...
      const canvasMeasurement = CanvasTextMetrics.CanvasTextMetrics.measureText(this.text, this._style);
      bounds[0] = 0;
      bounds[1] = 0;
  ...
  detectRenderType(style) {
    return Cache.Cache.has(style.fontFamily) ? "bitmap" : "canvas";
  }
  /**
