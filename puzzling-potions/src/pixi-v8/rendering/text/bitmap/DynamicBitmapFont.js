...
var _const = require('../../renderers/shared/texture/const.js');
...
var getCanvasFillStyle = require('../canvas/utils/getCanvasFillStyle.js');
...
class DynamicBitmapFont extends EventEmitter__default["default"] {
  constructor(options) {
    super();
    ...
    this.dynamic = true;
    let style;
    if (dynamicOptions.style instanceof TextStyle.TextStyle) {
      style = dynamicOptions.style.clone();
    } else {
      style = new TextStyle.TextStyle(dynamicOptions.style);
    }
  ...
  ensureCharacters(chars) {
    const charList = resolveCharacters.resolveCharacters(chars).filter((char) => !this.currentChars.includes(char)).filter((char, index, self) => self.indexOf(char) === index);
    if (!charList.length)
      return;
  ...
  nextPage() {
    ...
    const texture = new Texture.Texture({
      source: new ImageSource.ImageSource({
        ...
        resolution,
        alphaMode: _const.ALPHA_MODES.PMA
      })
    });
    const pageData = {
      canvasAndContext,
  ...
  setupContext(context, style, resolution) {
    ...
    if (style.dropShadow) {
      ...
      const dropShadowColor = convertColorToNumber.convertColorToNumber(shadowOptions.color);
      const rgb = hex.hex2rgb(dropShadowColor);
